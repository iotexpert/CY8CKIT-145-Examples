...
* 2 000 000 microseconds for PSoC 4000 / PSoC 4000S / 4100S / Analog 
* Coprocessor devices.<br>From 100 to 4 000 000 000 microseconds for 
* PSoC 4100 / PSoC 4200 / PSoC 4100 BLE / PSoC 4200 BLE / 
...
* CySysClkIloCompensate() should be called before WDT or WCO Timers enabling.
*
******************************************************************************/
...
* PSoC 4000 / PSoC 4100 / PSoC 4200 / PSoC 4000S / PSoC 4100S / 
...
* / PSoC 4000S / PSoC 4100S / Analog Coprocessor devices.
*
...
* LFCLK - PSoC 4100 BLE / PSoC 4200 BLE / PSoC 4100M / PSoC 4200M / PSoC 4200L.
*
* \note For PSoC 4000S / PSoC 4100S / Analog Coprocessor devices LFCLK can only 
* be sourced from ILO even though WCO is available.
*
* \param
* source
...
void CySysClkWcoClockOutSelect(uint32 clockSel)
    ...
    * is restored.
    *
    * Not applicable for the PSoC 4000 / PSoC4000S / PSoC 4100S / Analog 
    * Coprocessor devices.
    *
    * \details
    * This function has no effect if WDT is locked (CySysWdtLock() is
    ...
    *  functions is expected to be called inside a critical section.
    *
    * Not applicable for the PSoC 4000/PSoC 4000S/PSoC 4100S/ Analog 
    * Coprocessor devices.
    *
    * \details
    *  This function has no effect if WDT is locked (CySysWdtLock() is
...
********************************************************************************
*
* \brief Gets the clock source for the Timers.
*
* The function is applicable only for PSoC 4000 S / PSoC 4100 S / 
* Analog Coprocessor.
*
...
****************************************************************************//**
*
* \brief Sets the clock source for the Timers.
*
* The function is applicable only for PSoC 400S/ 4100S and Analog Coprocessor 
* devices.
*
* \param source
...
*
* \warning Timer reset is required if Timer source was switched while Timers 
* were running. Call CySysTimerResetCounters() API after Timer source switching.
* It is highly recommended to disable Timers before Timer source switching.
* Changing the Timer source may change the functionality that uses this Timers 
* as clock source.
*******************************************************************************/
    void CySysClkSetTimerSource(uint32 source)
    {
        ...
        if (CySysClkGetTimerSource() != source)
        {
            ...
            /* Wait 2 new clock source-cycles for change to come into effect */
            CyDelayUs(CY_SYS_2TIMER_DELAY_US);
    ...
    ****************************************************************************//**
    *
    * \brief Reads the enabled status of one of the three Timer counters.
    *
    ...
    *
    * \details
    * This function returns an actual Timer counter status from the status register.
    * It may take up to 3 LFCLK cycles for the Timer status register to contain 
    * actual data after the Timer counter is enabled.
    *
    *******************************************************************************/
    ...
    ****************************************************************************//**
    *
    * \brief Writes the mode of one of the three Timer counters.
    *
    * \param counterNum: Valid range [0-2]. The number of the Timer counter.
    * \param mode
    ...
    *
    * \details
    * Timer counter counterNum should be disabled to set a mode. Otherwise, this
    * function call has no effect. If the specified counter is enabled,
    * call the CySysTimerDisable() function with the corresponding parameter to
    * disable the specified counter and wait for it to stop.
    *
    *******************************************************************************/
    ...
    ****************************************************************************//**
    *
    * \brief Reads the mode of one of the three Timer counters.
    *
    * \param counterNum Valid range [0-2]. The number of the Timer counter.
    *
    * \return The mode of the counter. The same enumerated values as the mode 
    ...
    ****************************************************************************//**
    *
    * \brief Configures the Timer counter "clear on match" setting.
    *
    * If configured to "clear on match", the counter counts from 0 to MatchValue 
    * giving it a period of (MatchValue + 1).
    *
    ...
    ****************************************************************************//**
    *
    * \brief Reads the "clear on match" setting for the specified counter.
    *
    ...
    ****************************************************************************//**
    *
    * \brief Enables the specified Timer counters. All the counters specified in 
    * the mask are enabled.
    *
    ...
    ****************************************************************************//**
    *
    * \brief Disables the specified Timer counters.
    *
    * All the counters specified in the mask are disabled. The function waits for 
    * the changes to come into effect.
    *
    ...
    *
    * \brief
    * Writes the two Timers cascade values based on the combination of mask values
    * specified.
    *
    * \param cascadeMask The mask value used to set or clear the cascade values:
    ...
    ****************************************************************************//**
    *
    * \brief Reads the two Timer cascade values returning a mask of the bits set.
    *
    * \return The mask of the cascade values set.
    ...
    *
    * \brief
    * Reads a mask containing all the Timer counters interrupts that are currently
    * set by the hardware, if a corresponding mode is selected.
    *
    * \return The mask of interrupts set
    ...
    ****************************************************************************//**
    *
    * \brief Clears all the Timer counter interrupts set in the mask.
    *
    ...
    *
    * \brief
    * Sets the ISR callback function for the particular Timer counter.
    *
    ...
    *
    * \return The pointer to the previous callback function. 
    * \returnNULL is returned if the specified address is not set.
    *
    *******************************************************************************/
    ...
    ****************************************************************************//**
    *
    * \brief Gets the ISR callback function for the particular Timer counter.
    *
    * \param counterNum The number of the Timer counter.
    *
    * \return
    ...
    ****************************************************************************//**
    *
    * \brief This is the handler of the Timer interrupt in CPU NVIC.
    *
    ...
    * run as the first one and callback-2 is called as the last one.
    *
    * \details This function clears the Timer interrupt every time when it is called.
    *
    *******************************************************************************/
    ...
    * \details
    * This function waits while corresponding counters will be reset. This may
    * take up to 3 Timer source-cycles.
    * The Timer source must be enabled. Otherwise, the function will never exit.
    *
    *******************************************************************************/
    ...
    * PSoC 4200 BLE / PSoC 4200L/ PSoC 4100M/PSoC 4200M devices to use 
    * WDT. Also this API is available to use for PSoC 4000S / PSoC4100S / Analog 
    * Coprocessor devices to use WCO Timers.
    *
    * For PSoC 4100 / PSoC 4200 / PSoC 4100 BLE / PSoC 4200 BLE / PSoC 4200L / 
    * PSoC 4100M/PSoC 4200M devices:
    * The specified WDT counter should be configured as described below and started.
    *
    * For PSoC 4000S / PSoC4100S / Analog Coprocessor devices:
    * The specified WCO Timer counter should be configured as described below and 
    * started.
    *
    ...
    * disabled for the specified WDT or WCO Timer counter. Use 
    * CySysWdtSetClearOnMatch() for WDT or CySysTimerSetClearOnMatch() for WCO 
    * Timer function with the "enable" parameter equal to zero for the used WDT 
    * or WCO Timer counter.
    *
    ...
    *
    * Or the corresponding WCO Timer counter should be configured to match the 
    ...
    * WDT or WCO Timer counter.
    *
    * - Through the CySysWdtSetMode() for WDT or CySysTimerSetMode() for WCO 
    * Timer function. Call it with the appropriate "mode" parameter for the 
    * used WDT or WCO Timer counter.
    *
    * For the "INTERRUPT" mode, the recommended sequence is the following:
    * - Call the CySysWdtDisableCounterIsr() for WDT or 
    * CySysTimerDisableIsr() for WCO Timer function to disable servicing
    * interrupts of the specified WDT or WCO Timer counter.
    *
    * - Call the CySysWdtSetInterruptCallback() for WDT or 
    * CySysTimerSetIsrCallback() for WCO Timer function to register 
    * the callback function for the corresponding WDT or WCO Timer counter.
    *
    ...
    * from the corresponding WDT or WCO Timer counter. Servicing of this ISR 
    ...
    *
    * \brief
    * The function implements the delay specified as the number of WDT or WCO Timer 
    * clock source ticks between WDT or WCO Timer current value and match" 
    * value.
    *
    * This API is applicable for PSoC 4100 / PSoC 4200 / PSoC 4100 BLE / 
    * PSoC 4200 BLE / PSoC 4200L/ PSoC 4100M/PSoC 4200M devices to use WDT.
    * Also this API is available to use for PSoC 4000S / PSoC4100S / Analog 
    * Coprocessor devices to use WCO Timers.
    * 
    * For PSoC 4100 / PSoC 4200 / PSoC 4100 BLE / PSoC 4200 BLE / PSoC 4200L / 
    * PSoC 4100M/PSoC 4200M devices:
    ...
    *
    * For PSoC 4000S / PSoC4100S / Analog Coprocessor devices:
    * The function implements the delay specified as the number of WCO Timer input 
    * clock ticks for Timer0/Timer1 counter's current value and the "match" passed 
    * as the parameter to this function. The current WCO Timer counter value can 
    * be obtained using the CySysWdtGetCount() function.
    *
    * For PSoC 4100 / PSoC 4200 / PSoC 4100 BLE / PSoC 4200 BLE / PSoC 4200L / 
    * PSoC 4100M/PSoC 4200M devices:
    ...
    *
    * For PSoC 4000S / PSoC4100S / Analog Coprocessor devices:
    * Corresponding WCO Timer counter should be configured to match the 
    * selected mode: "Free running Timer" for the "WAIT" mode, and
    * "Periodic Timer" for the "INTERRUPT" mode.
    *
    * This can be configured in two ways:
    ...
    *
    * - Call the CySysWdtSetInterruptCallback() for WDT or 
    * CySysTimerSetInterruptCallback() for WCO Timer function to register 
    * the callback function for the corresponding WDT or WCO Timer counter.
    *
    * - Call the CySysTimerDelay() function.
    *
    * \param counterNum Valid range [0-1]. The number of the WDT or WCO Timer
